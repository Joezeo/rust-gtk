...
use adw::{prelude::*, subclass::prelude::*, traits::ActionRowExt, ActionRow, NavigationDirection};
use gtk::{
    gio::{self, Settings, SimpleAction},
    glib::{self, clone, BindingFlags, Object},
    pango,
    traits::EntryExt,
    Align, CheckButton, CustomFilter, FilterListModel, Label, ListBoxRow, NoSelection,
    SignalListItemFactory, Dialog, DialogFlags, ResponseType, Entry, SelectionMode,
};

use crate::util::data_path;
use crate::APP_ID;
use crate::{collection_object::CollectionData, task_row::TaskRow};
...
impl Window {
    ...
    fn setup_tasks(&self) {
        ...
        self.imp().tasks_list.bind_model(
            ...
            clone!(@weak self as window => @default-panic, move |obj| {
                let task_object = obj.downcast_ref().expect("The object should be type of `TaskObject`.");
    ...
    fn set_current_collection(&self, collection: CollectionObject) {
        ...
        let tasks_changed_handler_id =
            tasks.connect_items_changed(clone!(@weak self as window => move |tasks, _, _, _| {
                window.set_task_list_visible(tasks);
            }));
        self.imp()
